...
//			int result = 0; // 총 이익
			...
			for (int i = 0; i < N; i++) {
				...
				Table table = new Table(Integer.parseInt(tokenizer.nextToken()),
						Integer.parseInt(tokenizer.nextToken()), i);

				list.add(table);
			}

			builder.append("*" + getMaxProfit(N)).append("\n");
	...
	private static int getMaxProfit(int N) {
...
class Table {
	...
	public String toString() {
		return "Table [period=" + period + ", money=" + money + ", StartDate=" + getStartDate() + ", EndDate="
				+ getEndDate() + "]";
	}
}
